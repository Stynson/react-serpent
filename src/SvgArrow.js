...
|}): string {
    ...
    return `M${xStart},${yStart} ` + `${curveMarker}${xAnchor1},${yAnchor1} ${xAnchor2},${yAnchor2} ` + `${xEnd},${yEnd}`;
}
...
const SvgArrow = ({
    ...
    const endingPointWithArrow = computeEndingPointAccordingToArrowHead(endingPoint.x, endingPoint.y, actualArrowLength, strokeWidth, endingAnchorOrientation);
    const { xEnd, yEnd } = endingPointWithArrow;

    const startingPosition = computeStartingAnchorPosition(xStart, yStart, xEnd, yEnd, startingAnchorOrientation);
    const { xAnchor1, yAnchor1 } = startingPosition;

    const endingPosition = computeEndingAnchorPosition(xStart, yStart, xEnd, yEnd, endingAnchorOrientation);
